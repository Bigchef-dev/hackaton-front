...
const ConnectionIcon = {
  template: `<svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24 xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1"/></svg>`
}
...
<template>
  <div>
    ...
    <aside
      :class="[
        'hidden lg:flex flex-col fixed left-0 top-0 h-screen bg-gradient-to-b from-slate-900 to-slate-800 text-white transition-all duration-300 ease-in-out z-40 shadow-2xl',
    ...
    >
      ...
      <div class="flex items-center justify-between p-4 border-b border-slate-700">
        ...
        >
          ...
          <span class="font-bold text-xl whitespace-nowrap">Insérer nom app</span> //TODO
        </div>
        <button
          @click="toggleSidebar"
          class="p-2 hover:bg-slate-700 rounded-lg transition-colors duration-200 ml-auto"
      ...
      <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
        ...
        >
          <component :is="item.icon" class="w-5 h-5 flex-shrink-0 transition-transform duration-200" :class="{ 'group-hover:scale-110': activeRoute !== item.id }" />
          <span
            :class="[
      ...
      <!-- User Profile -->
      <div :class="['p-4 border-t border-slate-700', isSidebarOpen ? '' : 'flex justify-center']">
    ...
    <header class="lg:hidden fixed top-0 left-0 right-0 bg-slate-900 text-white shadow-lg z-50">
      <div class="flex items-center justify-between p-4">
        <div class="flex items-center space-x-3">
          ...
          <span class="font-bold text-lg">Insérer nom app</span> //TODO
        </div>
      ...
      >
        <nav v-show="isMobileMenuOpen" class="border-t border-slate-700 bg-slate-800 overflow-hidden">
          <button
            v-for="item in filteredMenuItems"
            :key="item.id"
            @click="navigateTo(item.id)"
    ...
    <!-- Main Content Slot -->
    <div :class="['pt-20 lg:pt-0 min-h-screen bg-slate-50 transition-all duration-300', isSidebarOpen ? 'lg:ml-64' : 'lg:ml-20']">
      <slot />
    </div>
  </div>
</template>